...
> P2 步骤：逐个确认后再替换，**本文档阶段不改代码**。

原则：Callable 应逐步消失，改为 **BookStore 方法** 或 **BookSnapshot 字段**。

---

## LlmHooks（3 个 Callable）

| 字段 | main 注入 | 功能 | 新架构归属 |
|------|-----------|------|------------|
| `build_cached_system` | `main.build_cached_system` | 组装 Prompt Cache system 块 | 保留为 `LlmClient` 适配器（依赖 context，暂留 main） |
| `call_api` | `main.call_api` | 统一 LLM 调用 | `core.api.complete` 薄封装 |
| `get_last_call_info` | `main.get_last_call_info` | 费用/错误元数据 | `core.api` 或 session 状态 |
...
| 字段 | main 注入 | 功能 | 新架构归属 |
|------|-----------|------|------------|
| `get_char_context_for_check` | `get_char_context_for_check` | 检查用人设块 | `BookSnapshot.char_context_for_check`（`load_snapshot(for_purpose="check")`） |
...
| `get_chapters_text_for_scope` | `get_chapters_text_for_scope` | 多章正文拼接 | `BookStore.chapters_text_for_scope(num, scope)` |
...
| `write_append_locked` | lambda → `write_text(PLOT_THREADS_LOCKED...)` | 追加钉子 | `BookStore.append_locked(num, text)` |
...
| `extract_plot_unresolved` | `extract_plot_active_unresolved` | 提取未回收伏笔段 | `BookSnapshot.plot_unresolved`（load_snapshot 内计算） |
...
2. `read_*` / `get_summaries_*` / `extract_plot_unresolved` → `BookStore.load_snapshot`（去掉 `import main`）
3. 删除 `MaintainDeps` 中已可选的 Callable 字段定义
4. `ReviewerDeps` 其余 Callable → `BookSnapshot` + `BookStore`
5. `LlmHooks` 最后收敛到 `core.api`
