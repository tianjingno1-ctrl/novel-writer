...
> P2 步骤：逐个确认后再替换。**LlmHooks 已迁至 `app.llm`（P3-5a）**。
...
| 字段 | 注入来源 | 功能 | 现状 |
|------|-----------|------|------|
| `build_cached_system` | `app.llm.build_cached_system` | 组装 Prompt Cache system 块 | `app/factories._llm_hooks()` |
| `call_api` | `app.llm.call_api` | 统一 LLM 调用 | 同上；`core.api.complete` 薄封装 |
| `get_last_call_info` | `app.llm.get_last_call_info` | 费用/错误元数据 | 同上 |
...
5. `LlmHooks` 收敛到 `core.api`（可选，低优先级）
