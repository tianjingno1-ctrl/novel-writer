...
1. 先在 `world.md` 和 `characters.md` 写好基础设定。
2. 在 `data/chapters/` 创建 `ch001.md` 并开始写作。
3. 每写完一章，运行 `/summary` 生成概述。
4. **每写完 3–5 章，运行一次 `/check` 检查矛盾**，及早发现设定冲突。
5. 有新人物或设定变更时，用 `/patch` 追加到 characters.md，不要直接改旧内容。
